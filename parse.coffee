...
make_parse = ->
  ...
  original_scope = 
    define: (n) ->
      ...
      n.reserved = false
      n.nud = itself
      #A nud method is used by values and by prefix operators.
      #A nud does not care about the tokens to the left.
      n.led = null
      n.std = null
      #callback parsing the kind of statement. Used at the beginning of the staement
      n.lbp = 0
  ...
  symbol = (id, bp) ->
    # id = string, bp = binding power
    s = symbol_table[id]
    bp = bp or 0
    if s
      if bp >= s.lbp
        # update left binding power
        s.lbp = bp
    else
  ...
  stmt = (s, f) ->
    #s es string, f es funcion anonima.
    x = symbol(s)
    #x es el simbolo equivalente de s.
    x.std = f
    #el atributo std de x sera f
    x
    #devolvemos el sym con su std modificado.
